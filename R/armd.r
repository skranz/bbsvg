...
armd.parse.pane = function(bi, am, inner.txt = get.bi.inner.txt(bi=bi, am=am), args = get.bi.args(bi=bi, am=am), name=args$name, ...) {
  ...
  pane = init.yaml.pane(yaml=yaml, direct=TRUE, name=name)
...
armd.parse.plotpane = function(bi, am, type="pane",inner.txt = get.bi.inner.txt(bi=bi, am=am), args = get.bi.args(bi=bi, am=am), name=args$name, ps=am, ...) {
  ...
  # use cache
  hash = digest(list(pane$data, pane$pane.yaml, yaml))
  cache.dir = first.non.null(am$cache.dir,file.path(getwd(),"cache"))
  cache.file = file.path(cache.dir,paste0("plotpane_",hash,".rds"))
  ...
  if (file.exists(cache.file)) {
    ...
    html = res$html
    has.latex = any(sapply(pane$objs, function(obj) "latex" %in% names(obj)))
    if (has.latex) {
      res$svg.html = latexsvg(html)
    } else {
      res$svg.html = html
    }
