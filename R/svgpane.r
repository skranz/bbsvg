...
place.svg.pane.labels = function(svg, geoms, pane, left.offset=NULL, bottom.offset=NULL, label.df = NULL, label.postfix, display=NULL, left.outside=TRUE, bottom.outside=TRUE) {
  ...
  force.inside = sapply(geoms, function(geom) geom$type != "marker")
  left.outside = left.outside & !force.inside
  bottom.outside = bottom.outside & !force.inside
  

  
  anchor = ifelse(is.right,"end",ifelse(is.left,"start", "middle"))
  anchor[left.outside & is.left] = "end"  
  
  left.offset.inside = 5
  left.offset.outside = -5
  bottom.offset.outside = 20
  bottom.offset.inside = -5
    
  rp$x[is.right] = pane$width-3
  
  rp$x[is.left & left.outside] = rp$x[is.left & left.outside] + left.offset.outside
  rp$x[is.left & !left.outside] = rp$x[is.left & !left.outside] + left.offset.inside
  
  rp$y[is.bottom & bottom.outside] = rp$y[is.bottom & bottom.outside] + bottom.offset.outside
  rp$y[is.bottom & !bottom.outside] = rp$y[is.bottom & !bottom.outside] + bottom.offset.inside
  
  # inside labels are above y-axis
  rp$y[force.inside] = pmin(rp$y[force.inside],svg$dr$range$y["bottom"]+bottom.offset.inside)
  ...
  for (r in seq_len(NROW(label.df))) {
    ...
    if (isTRUE(geom$obj$label.mode=="latex")) {
      align = ""
      if (anchor[r]=="end") align="R"
      if (anchor[r]=="start") align="L"
      
      svg_mathjax_label(svg,x=rp$x[r],y=rp$y[r], text=labels[ind],id=paste0("geomlabel_",geom$id), level=100,to.range = FALSE, align=align, tooltip = geom$tooltip, display=display)
      
    } else {
      svg_boxed_label(svg,x=rp$x[r],y=rp$y[r], text=labels[ind],id=paste0("geomlabel_",geom$id), level=100,to.range = FALSE, "text-anchor"=anchor[r], tooltip = geom$tooltip, display=display)
